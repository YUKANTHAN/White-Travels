...
class DeepConcierge:
    ...
    CITY_DB = {
        # --- JAPAN & EAST ASIA ---
        'tokyo':        {'country': 'Japan',        'has_rail': True, 'attractions': 'Shibuya Crossing, Senso-ji Temple, Akihabara, Imperial Palace, Harajuku', 'food': 'Ramen, Sushi, Tempura, Mochi', 'airline': 'ANA / Japan Airlines', 'language': 'Japanese', 'currency': 'Japanese Yen (JPY)', 'best_time': 'March-May or Oct-Nov', 'safety': '9.2/10 - one of the safest cities on Earth'},
        ...
        'cabo':         {'country': 'Mexico',       'has_rail': False, 'attractions': 'El Arco Rock Arch, Medano Beach, Lovers Beach, Glass Beach, Pacific Side', 'food': 'Tacos, Tamales, Margaritas, Fish Tacos', 'airline': 'Aeromexico', 'language': 'Spanish', 'currency': 'Mexican Peso (MXN)', 'best_time': 'Nov-May', 'safety': '7.0/10'},
        'honolulu':     {'country': 'USA (Hawaii)', 'has_rail': False, 'attractions': 'Waikiki Beach, Diamond Head, Pearl Harbor, Hanauma Bay, Road to Hana', 'food': 'Poke Bowl, Spam Musubi, Shave Ice, Lomi Lomi Salmon', 'airline': 'Hawaiian Airlines', 'language': 'English', 'currency': 'US Dollar (USD)', 'best_time': 'Apr-Jun or Sep-Nov', 'safety': '8.5/10'},
        # --- MIDDLE EAST & AFRICA ---
        ...
        'bali':         {'country': 'Indonesia',    'has_rail': False, 'attractions': 'Tanah Lot, Ubud Rice Terraces, Uluwatu Temple, Kuta Beach', 'food': 'Nasi Goreng, Satay, Babi Guling, Mie Goreng', 'airline': 'Garuda Indonesia', 'language': 'Indonesian', 'currency': 'Indonesian Rupiah (IDR)', 'best_time': 'May-Sep', 'safety': '7.5/10'},
        # --- INDIA (EXPANDED) ---
        ...
        'ooty':         {'country': 'India',        'has_rail': False, 'attractions': 'Ooty Lake, Botanical Gardens, Nilgiri Mountain Railway (UNESCO), Doddabetta Peak, Tea Estates, Pykara Falls', 'food': 'Varkey (Ooty Biscuit), Homemade Chocolates, Nilgiri Tea, Mushroom Soup', 'airline': 'IndiGo (via Coimbatore, 3hrs drive)', 'language': 'Tamil / Badaga', 'currency': 'Indian Rupee (INR)', 'best_time': 'Oct-June (avoid monsoons)', 'safety': '9.0/10 - Hill station, very safe'},
        'kodaikanal':   {'country': 'India',        'has_rail': False, 'attractions': 'Kodaikanal Lake, Coaker Walk, Pillar Rocks, Bryant Park, Silver Cascade Falls, Dolphin Nose', 'food': 'Homemade Chocolates, Eucalyptus Oil products, Fresh Mushroom dishes, Bajji', 'airline': 'IndiGo (via Madurai, 3hrs drive)', 'language': 'Tamil', 'currency': 'Indian Rupee (INR)', 'best_time': 'Oct-June', 'safety': '9.0/10 - Very peaceful'},
    ...
    def plan_trip(self, data):
        ...
        # Check if the city has rail facilities
        city_info = self.CITY_DB.get(dest.lower(), {})
        has_rail = city_info.get('has_rail', True) # Default to true for unknown cities
        ...
        if transport == 'Flight':
            ...
            if has_rail:
                ...
                plan += f"• **{transport} Unavailable:** Notice — {dest} does not have direct high-speed rail access. I have automatically pivoted your transport to a **Premium Inter-City Shuttle** for the final leg.\n"
        plan += "\n"
